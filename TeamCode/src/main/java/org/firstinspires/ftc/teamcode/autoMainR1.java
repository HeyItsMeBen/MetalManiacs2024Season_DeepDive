    ...
    public void runOpMode() {
     ...
     camera = OpenCvCameraFactory.getInstance().createWebcam(hardwareMap.get(WebcamName.class, "Webcam 1"), cameraMonitorViewId);*/      //commented
        ...
        if (testMode==4){
            ...
            slides.open_close_outtake(0.25, 0.25);
            sleep(1000);
        }
        if (testMode==3){
           ...
           */
            ...
            sleep(1000);
            drive1.moveClockwiseTurn(fullCircle*0.5, dblPower);  //GET compDrive LATEST
            sleep(1000);
            /*Code for Claw and slides*/ //needs to be 0.25 SPEED (i think)
            //scores sample onto bar
            slides.extendVertical(0.25);      //more power?
            sleep(50);          //100-->30-->
            slides.extendVertical(0);
            ...
            for (int i=0; i<1; i++) {
                ...
                claw.open_close(0.5,0.5);   //0-->0.5        //unneccessary
                sleep(1000);
                claw.moveArm(-0.25);    //higher power?
                sleep(1000);
                claw.open_close(0.75, 0.75); //1(or0)-->0.75
                sleep(1000);
                ...
                //move to basket
                if (i==0){              //different. Make it same as if-statement above
                    drive1.moveLeft(tileLength*0.33333*1.33333, dblPower);
                }
                else {
                    drive1.moveRight((i-1)*tileLength*0.33333*1.33333, dblPower);       //is it POSsiBLE to move rght by 0?
                }
                sleep(1000);
